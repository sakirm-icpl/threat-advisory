...
export default function AdminPanel() {
  ...
  if (user?.role !== 'admin') {
    return (
      <div className="max-w-2xl mx-auto mt-16">
        <div className="bg-slate-800/95 backdrop-blur-sm rounded-2xl p-12 text-center border border-slate-700 shadow-cyber">
          <ExclamationTriangleIcon className="mx-auto h-16 w-16 text-yellow-400" />
          <h2 className="mt-4 text-xl font-semibold text-slate-100">Access Denied</h2>
          <p className="mt-2 text-slate-400">You need admin privileges to access this page.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
            ...
            <table className="min-w-full divide-y divide-slate-700">
              ...
              <tbody className="bg-slate-800/50 divide-y divide-slate-700">
                {usersData?.map((userRow) => (
                  <tr key={userRow.id} className="hover:bg-slate-700/50">
                        ...
                        <div className="flex justify-end space-x-2">
                          {userRow.role === 'contributor' ? (
                            <button
                              ...
                              className="inline-flex items-center px-3 py-1 border border-red-500/50 text-xs font-medium rounded-md text-red-300 bg-red-500/20 hover:bg-red-500/30 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors duration-200"
                            >
                            ...
                            <button
                              ...
                              className="inline-flex items-center px-3 py-1 border border-slate-500/50 text-xs font-medium rounded-md text-slate-300 bg-slate-500/20 hover:bg-slate-500/30 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 transition-colors duration-200"
                            >
      ...
      {/* Confirmation Dialog */}
      {showConfirmDialog && (
        <div className="fixed inset-0 bg-slate-900/70 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
          <div className="relative bg-slate-800 rounded-lg shadow-xl max-w-md w-full mx-4 border border-slate-700">
            <div className="p-6">
              <div className="flex items-center">
                <div className={`flex-shrink-0 ${showConfirmDialog.type === 'promote' ? 'text-red-400' : 'text-yellow-400'}`}>
              ...
              <div className="mt-6 flex justify-end space-x-3">
                ...
                <button
                  ...
                  className={`px-4 py-2 border border-transparent rounded-md text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                    showConfirmDialog.type === 'promote'
                      ? 'bg-red-500/20 hover:bg-red-500/30 border border-red-500/30 focus:ring-red-500'
                      : 'bg-yellow-500/20 hover:bg-yellow-500/30 border border-yellow-500/30 focus:ring-yellow-500'
                  }`}
                >
